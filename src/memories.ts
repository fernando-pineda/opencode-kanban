...
const DB_PATH =
  process.env.OPENCODE_MEMORIES_DB_PATH ||
 ...
 */
export function repoToTableName(repoPath: string): string {
 ...
 */
export function ensureRepoTable(repoPath: string): string {
...
export function saveMemoryWithEmbedding(
  repoPath: string,
  {
    ...
    importance = 0.5,
    embedding,
  }: MemorySaveOptions & { embedding: Float32Array | number[] },
): number {
  const database = getDb();
  const tableName = ensureRepoTable(repoPath);
  ...
  if (embedding && (embedding as unknown[]).length === 768) {
...
export function searchMemories(
  ...
  return database.prepare(sql).all(...params) as Memory[];
}
  ...
  for (const vecRow of vecRows) {
    if (vecRow.distance <= threshold) {
      const mapping = database
        ...
        .get(vecRow.rowid) as { memory_id: number } | undefined;

      if (mapping) {
        const memory = database.prepare(`SELECT * FROM ${tableName} WHERE id = ?`).get(mapping.memory_id) as Memory;
        if (memory) {
 ...
 */
export function getMemoriesStats(repoPath: string): { total: number; by_type: Array<{ memory_type: string; count: number }> } {
  const database = getDb();
  const tableName = ensureRepoTable(repoPath);
 ...
 */
export function getMemoriesCount(repoPath: string, { memoryType }: { memoryType?: string } = {}): number {
  const database = getDb();
  const tableName = ensureRepoTable(repoPath);
 ...
 */
export function getRepos(): Array<{ repo_id: string; repo_path: string; created_at: string; last_accessed: string }> {
  return getDb()
    .prepare('SELECT * FROM _meta ORDER BY last_accessed DESC')
    .all() as Array<{ repo_id: string; repo_path: string; created_at: string; last_accessed: string }>;
}
...
export function pruneMemories(
  repoPath: string,
  { keepDays = 90, keepImportant = true }: { keepDays?: number; keepImportant?: boolean } = {},
): number {
  const database = getDb();
  const tableName = ensureRepoTable(repoPath);
 ...
 */
export function compactConversationMemories(
  repoPath: string,
  conversationId: string,
): { original: number; compacted: number; removed: number } {
  const database = getDb();
  const tableName = ensureRepoTable(repoPath);
 ...
 */
export function batchSaveKnowledge(
  repoPath: string,
  entries: KnowledgeSaveOptions[],
  indexedBy?: string,
): number {
  const database = getDb();
  const tableName = ensureRepoTable(repoPath);
  ...
  const transaction = database.transaction((items: KnowledgeSaveOptions[]) => {
...
export function searchKnowledge(
  repoPath: string,
  {
    ...
    category,
    limit = 30,
  }: KnowledgeSearchOptions & { limit?: number } = {
    query: '',
  },
): KnowledgeEntry[] {
  const database = getDb();
  ...
  const results = database.prepare(sql).all(...params) as KnowledgeEntry[];
...
export function getKnowledge(
  repoPath: string,
  { category, key }: { category: string; key: string },
...
export function listKnowledge(
  ...
  const rows = database.prepare(sql).all(...params) as KnowledgeEntry[];
 ...
 */
export function getKnowledgeStats(repoPath: string): KnowledgeStats {
  const database = getDb();
  const tableName = ensureRepoTable(repoPath);
  const knName = `${tableName}_knowledge`;
  ...
  const topAccessed = database
    .prepare(
      `SELECT category, key, title, access_count FROM ${knName} ORDER BY access_count DESC LIMIT 15`,
    )
    .all() as Array<{ category: string; key: string; title: string; access_count: number }>;
 ...
 */
export function getStaleKnowledge(
  repoPath: string,
  {
    olderThanDays = 7,
    category,
    limit = 100,
  }: { olderThanDays?: number; category?: string; limit?: number } = {},
): KnowledgeEntry[] {
  const database = getDb();
  const tableName = ensureRepoTable(repoPath);
  const knName = `${tableName}_knowledge`;
  ...
  return database.prepare(sql).all(...params) as KnowledgeEntry[];
}
