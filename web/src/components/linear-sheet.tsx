...
function priorityColor(priority: number): string {
  switch (priority) {
    ...
    default:
      return "#9ca3af"; // No priority — gray
  }
}
...
export default function LinearSheet({
  boardId,
  open,
  onOpenChange,
}: LinearSheetProps) {
  ...
  const filteredIssues = issues.filter((i) => {
    // Priority filter
    if (priorityFilter !== "all" && i.priority !== parseInt(priorityFilter))
      return false;
    // Assignee filter
  ...
  const filteredTeams = teamFilter
    ? teams.filter(
        (t) =>
          ...
          (t.description &&
            t.description.toLowerCase().includes(teamFilter.toLowerCase())),
      )
    : teams;
              ...
              <div className="p-6 space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Enter a Linear Personal API Key. The key is stored locally
                    for this board only.
                  </p>
                ...
                <div className="px-6 pt-4 pb-3 border-b">
                  ...
                  {detailIssue.assignee && (
                    <div className="flex items-center gap-1 mt-2">
                      ...
                      >
                        {detailIssue.assignee.display_name ||
                          detailIssue.assignee.name}
                      </Badge>
                    </div>
                  )}
                  {detailIssue.project && (
                      ...
                      >
                        <div className="space-y-1.5">
                          ...
                          <Select
                            ...
                            onValueChange={setStateTypeFilter}
                          >
