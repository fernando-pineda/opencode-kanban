...
export default function GithubSheet({
  boardId,
  open,
  onOpenChange,
}: GithubSheetProps) {
  ...
  // Issues state
  const [issuesByRepo, setIssuesByRepo] = useState<
  ...
  const [issuesStateFilter, setIssuesStateFilter] = useState<
    "open" | "closed" | "all"
  >("open");
  const [issuesAssigneeFilter, setIssuesAssigneeFilter] =
    useState<string>("all");
  ...
  const [projectsAssigneeFilter, setProjectsAssigneeFilter] =
    useState<string>("all");
  ...
  const [detailProjectItem, setDetailProjectItem] =
    useState<GitHubProjectItem | null>(null);
  ...
  const fetchProjectPicker = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/boards/${boardId}/github/projects?all=true`,
      );
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchProjectItems = useCallback(
    async (projectId: string) => {
      ...
      try {
        const res = await fetch(
          `/api/boards/${boardId}/github/projects/${projectId}/items`,
        );
        if (res.ok) {
          const data = await res.json();
  ...
  const currentRepoIssues = activeRepoTab
    ? issuesByRepo[activeRepoTab] || []
    : [];
  const filteredIssues = currentRepoIssues.filter((i) => {
    // State filter
    if (issuesStateFilter !== "all" && i.state !== issuesStateFilter)
      return false;
    // Assignee filter
    if (issuesAssigneeFilter !== "all") {
      const hasAssignee = i.assignees.some(
        (a) => a.login === issuesAssigneeFilter,
      );
    ...
    if (issuesFilter) {
      return (
        ...
        i.labels.some((l) =>
          l.name.toLowerCase().includes(issuesFilter.toLowerCase()),
        )
      );
    }
    return true;
  ...
  const uniqueAssignees = Array.from(
    new Map(
      currentRepoIssues.flatMap((i) => i.assignees.map((a) => [a.login, a])),
    ).values(),
  );
  const uniqueLabels = Array.from(
    new Map(
      currentRepoIssues.flatMap((i) => i.labels.map((l) => [l.name, l])),
    ).values(),
  );

  const issuesActiveFilterCount = [
    issuesStateFilter !== "open",
    issuesAssigneeFilter !== "all",
    issuesLabelFilter !== "all",
  ].filter(Boolean).length;
  const uniqueProjectAssignees = Array.from(
    new Map(
      projectItems.flatMap((i) => i.assignees.map((a) => [a.login, a])),
    ).values(),
  );
  const projectsActiveFilterCount = [
    projectsTypeFilter !== "all",
    projectsAssigneeFilter !== "all",
  ].filter(Boolean).length;
  ...
  const filteredProjectItems = projectItems.filter((i) => {
    // Type filter
    if (projectsTypeFilter !== "all" && i.type !== projectsTypeFilter)
      return false;
    // Assignee filter
    if (projectsAssigneeFilter !== "all") {
      const hasAssignee = i.assignees.some(
        (a) => a.login === projectsAssigneeFilter,
      );
      if (!hasAssignee) return false;
    }
    // Text filter
    if (projectItemsFilter) {
      return (
        ...
        (i.status &&
          i.status.toLowerCase().includes(projectItemsFilter.toLowerCase())) ||
        (i.repository &&
          i.repository.toLowerCase().includes(projectItemsFilter.toLowerCase()))
      );
    }
    return true;
  });
              ...
              <div className="p-6 space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Enter a GitHub Personal Access Token (classic). The token is
                    stored locally for this board only.
                  </p>
                    ...
                    <div ref={repoScrollRef} className="h-full overflow-y-auto">
                      ...
                      >
                        {repoVirtualizer.getVirtualItems().map((virtualRow) => {
                          ...
                          if (!repo) return null;
                          const isSelected = selectedRepos.includes(
                            repo.full_name,
                          );
                          return (
                            <button
                ...
                <div className="flex-1 overflow-hidden px-6">
                  {loadingProjects ? (
                    ...
                    >
                      ...
                      >
                        {projectPickerVirtualizer
                          .getVirtualItems()
                          .map((virtualRow) => {
                            const project =
                              filteredPickerProjects[virtualRow.index];
              ...
              <div className="flex flex-col h-full">
                <div className="px-6 pt-4 pb-3 border-b">
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    <Badge
                      variant={
                        detailIssue.state === "open" ? "default" : "secondary"
                      }
                      className="text-[10px] px-1.5 py-0"
                    >
                      {detailIssue.state}
                    ...
                    <span className="text-xs text-muted-foreground">
                      opened {formatTimeAgo(detailIssue.created_at)} by{" "}
                      {detailIssue.user.login}
                    </span>
                    ...
                    <Popover>
                      ...
                      >
                        <div className="space-y-1.5">
                          ...
                          <Select
                            value={issuesStateFilter}
                            onValueChange={(v) =>
                              setIssuesStateFilter(
                                v as "open" | "closed" | "all",
                              )
                            }
                          >
                        ...
                        <div className="space-y-1.5">
                          ...
                          <Select
                            value={issuesAssigneeFilter}
                            onValueChange={setIssuesAssigneeFilter}
                          >
                            <SelectTrigger className="h-7 text-xs">
                              <SelectValue />
                            </SelectTrigger>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <Select
                            ...
                            onValueChange={setIssuesLabelFilter}
                          >
                            <SelectTrigger className="h-7 text-xs">
                              <SelectValue />
                            </SelectTrigger>
                  ...
                  <div className="flex flex-col h-full">
                    <div className="px-6 pt-4 pb-3 border-b">
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => setDetailProjectItem(null)}
                        className="h-7 px-2 text-xs"
                      >
                        ← Back
                      </Button>
                      <h2 className="text-base font-semibold mt-2">
                        {detailProjectItem.title}
                      </h2>
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        ...
                        {detailProjectItem.type && (
                          <Badge
                            ...
                            className="text-[10px] px-1.5 py-0"
                          >
                            {detailProjectItem.type === "PULL_REQUEST"
                              ? "PR"
                              : detailProjectItem.type}
                          </Badge>
                        )}
                    ...
                    <div className="border-t px-6 py-4 flex items-center gap-2">
                      {detailProjectItem.html_url && (
                        <Button
                          variant="outline"
                          onClick={() =>
                            window.open(detailProjectItem.html_url!, "_blank")
                          }
                          className="text-xs"
                        >
                          <ExternalLink className="h-3 w-3 mr-1" />
                      ...
                      <Button
                        ...
                        onClick={() => {
                          setSpawnIssue({
                            ...
                            repository_url: detailProjectItem.repository
                  ...
                  <div className="flex flex-col h-full">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="relative flex-1">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setProjectItemsFilter(e.target.value)
                            }
                            className="pl-8 h-8 text-xs"
                          />
                        </div>
                          ...
                          >
                            <div className="space-y-1.5">
                              ...
                              <Select
                                ...
                                onValueChange={setProjectsTypeFilter}
                              >
                                <SelectTrigger className="h-7 text-xs">
                                  <SelectValue />
                                </SelectTrigger>
                            ...
                            <div className="space-y-1.5">
                              ...
                              <Select
                                value={projectsAssigneeFilter}
                                onValueChange={setProjectsAssigneeFilter}
                              >
                                <SelectTrigger className="h-7 text-xs">
                                  <SelectValue />
                                </SelectTrigger>
                    ...
                    >
                      {loadingProjectItems ? (
                        ...
                        <div
                          style={{
                            height: `${projectItemVirtualizer.getTotalSize()}px`,
                            width: "100%",
                            position: "relative",
                          }}
                        >
                          {projectItemVirtualizer
                            .getVirtualItems()
                            .map((virtualRow) => {
                              const item =
                                filteredProjectItems[virtualRow.index];
                              if (!item) return null;
                              return (
                                <div
                                  key={item.id}
